...
class FinanceController extends Controller
{
    ...
    public function printApproval(Request $request,$id)
    {
        $data = PurchaseApprovalRequest::find($id);
        if(!$data)
        {
            return response()->json(['Not found.'],404);
        }

        return view('finance.print.purchase_approval',compact('data'));
    }
    ...
    public function downloadApproval(Request $request,$id)
    {
        ...
        if(!$data)
        {
            return response()->json(['Record not found. id:' . $id],404);
        }

        $filename = 'finance_purchase_approval_request_' . now()->format('Ymd_His') . '.pdf';

        $token = env('BROWSERLESS_API_TOKEN');
        $printUrl = url("/finance/print_approval/{$id}");
